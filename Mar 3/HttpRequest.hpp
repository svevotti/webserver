...
public:
    ...
    void parseRequestHttp(); // No 'void' parameter, just empty ()
    void parseRequestLine(std::string);
    void parseHeaders(std::istringstream&);
    void parseBody(std::string buffer, int size, std::istringstream& str);
    ...
    std::string getHttpBodyText() const; // Declared but not implemented

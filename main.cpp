...
bool test(const Config &conf)
{
    ...
    i = 0;
    std::vector<InfoServer*> server = conf.getServList();
    if (server.size() == 0)
        return false;
    for(servIt = server.begin(); servIt != server.end(); servIt++)
    {
        ...
        std::map<std::string, std::string> map = (*servIt)->getSetting();
        for(mapIt = map.begin(); mapIt != map.end(); mapIt++)
            std::cout << mapIt->first << " = " << mapIt->second << std::endl;

        std::cout << std::endl << "Checking all locations" << std::endl << std::endl;

        std::map<std::string, Route> routes = (*servIt)->getRoute();

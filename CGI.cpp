...
void CGI::createAv(const std::string& cgi_path, const std::string& upload_to)
{
	...
	if (!(_serverInfo.getCGI().locSettings["cgi_extension"].find(extension)))
		throw CGIException("Unsupported interpreter for extension: " + extension + "; Please adjust configuration file");
	if (extension == ".py")
		_av[0] = strdup("/usr/bin/python3");
	else if (extension == ".php")
		_av[0] = strdup("/usr/bin/php-cgi");
	else
		throw CGIException("Unable to run extension " + extension);

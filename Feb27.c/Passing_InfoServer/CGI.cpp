...
std::string CGI::getScriptFileName(const ClientRequest& request, const InfoServer& info) const
{
    ...
    std::string cgi_root = "/www/cgi-bin/";  // Hardcoded for testing—InfoServer lacks full root
    if (script_name[0] != '/')
...
void CGI::createAvAndEnv(const ClientRequest& request, const InfoServer& info)
{
    ...
    _env[i++] = strdup("SERVER_NAME=127.0.0.1");  // Hardcoded for testing
    _env[i++] = strdup("SERVER_PORT=8080");       // Hardcoded for testing
...
// decodes a "chunked" HTTP request body into a single, continuous string
// for handling POST requests
// We need it ONLY IF the server supports clients sending POST requests with 
/// Transfer-Encoding: chunked (a standard HTTP/1.1 feature),
// to decode that data before passing it to CGI.
// Currently, CGI::processRequestBody checks for chunked encoding
...
void CGI::populateEnvVariables(const ClientRequest& request, const InfoServer& info)
{
    ...
    _env_var["SERVER_PORT"] = "8080";  // Hardcoded for testing
    _env_var["SERVER_NAME"] = "127.0.0.1";  // Hardcoded for testing
}

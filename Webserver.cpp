...
void Webserver::dispatchEvents()
{
    ...
    for (it = poll_sets.begin(); it != ite;) 
    {
        ...
        if (it->revents & POLLIN)
        {
			...
			{
				int i = retrieveClientIndex(it->fd);
				if (i != -1)
					result = this->clientList[i]->processClient();
...
{
	int i = retrieveClientIndex(fd);
	if (i == -1)
	{
		Logger::debug("Client not found for sending response");
		return;
	}
	//TODO:does it behave as recv?
	//Logger::debug("bytes to send " + std::to_string(iterClient->response.size()));
	int bytes = send(fd, this->clientList[i]->getResponse().c_str(), strlen(this->clientList[i]->getResponse().c_str()), 0);
	if (bytes == -1)
...
void Webserver::createNewClient(int fd)
{
	...
	clientPoll.events = POLLIN;
	this->poll_sets.push_back(clientPoll);
	Client *client = new Client(clientFd, *this->_serverInfo);

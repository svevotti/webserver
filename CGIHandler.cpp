...
void CGIHandler::startCGI(ClientHandler& client, const InfoServer& serverConfig, std::vector<struct pollfd>& poll_sets)
{
    ...
    if (contentLength > maxSize) 
    {
        ...
        client.setResponse(createStandardResponse(413, errorBody, contentType, true).composeRespone());
    ...
    if (access(cgi_path.c_str(), F_OK) != 0) 
    {
        ...
        client.setResponse(createStandardResponse(404, errorBody, contentType, true).composeRespone());
        addPipeToPoll(client.getFd(), POLLOUT, poll_sets);
    ...
    if (access(cgi_path.c_str(), X_OK) != 0)
    {
        ...
        client.setResponse(createStandardResponse(403, errorBody, contentType, true).composeRespone());
        addPipeToPoll(client.getFd(), POLLOUT, poll_sets);
    ...
    if (routeIt != routes.end()) 
    {
        ...
        if (!isAllowed) // 501 Not Implemented if extension is not allowed
        {
            ...
            client.setResponse(createStandardResponse(501, errorBody, contentType, true).composeRespone());
            addPipeToPoll(client.getFd(), POLLOUT, poll_sets);
    ...
    catch (const CGIException& e)
    {
        ...
        client.setResponse(createStandardResponse(500, errorBody, contentType, true).composeRespone());
...
std::string CGIHandler::handleFortuneResponse(std::vector<CGITracker>::iterator cgiIt, bool isComplete) 
{
    ...
    if (cgiIt->statusCode >= 400) // Error cases
    {
        ...
        std::string fullResponse = response.composeRespone();
                                      ...
                                      bool isComplete, bool final) 
{
    ...
    else if (!isComplete && cgiIt->statusCode == 200 && cgiIt->bufferedOutput.length() < 4096 && !cgiIt->isChunked) 
    {
        ...
        std::string initialResponse = response.composeRespone();
        std::ostringstream chunk;
...
std::string CGIHandler::generateServerTimeoutResponse(std::vector<CGITracker>::iterator cgiIt) 
{
    ...
    else 
    {
        ...
        fullResponse = response.composeRespone();
    ...
    for (; clientIt != clients.end(); ++clientIt)
    {
        if (clientIt->getFd() == cgiIt->clientFd)
        {
            if (clientIt->getResponse().empty())
            {
                ...
                if (isSigInt) 
                    clientIt->setResponse(createStandardResponse(200, "Request cancelled by user", "text/plain", true).composeRespone());
                else 
                    clientIt->setResponse(createStandardResponse(500, "CGI execution failed", "text/plain", true).composeRespone());

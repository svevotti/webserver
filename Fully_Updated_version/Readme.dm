...
2. Included some forward declarations in CGI and in ServerSocket (one class by H - Server - and two by Sveva -- ClientResponse and ServerResponse)
...
// isn’t poll()-gated here.

// Diagnosis: send() is allowed, but needs poll() protection.

...
const std::string CGI::SCRIPT_BASE_DIR = "/www/cgi-bin/";  // Fallback only? To be discussed 
...
CGI::CGI(const ClientRequest& request, const std::string& upload_to, const Server& server)
    : _request_method(request.getRequestLine()["Method"]), _upload_to(upload_to),
      ...
      _input_done(false), _output_done(false), _poll_fds(), _start_time(time(NULL))
{
    try 
    {
...
std::string CGI::getScriptFileName(const ClientRequest& request, const Server& server) const
{
    ...
    std::string cgi_root = server.getRoot() + "/cgi-bin/";  // Use Server::_root
    if (script_name[0] != '/')
...
void CGI::createAvAndEnv(const ClientRequest& request, const Server& server)
{
    ...
    // Build _env from ClientRequest and Server
    ...
    _env[i++] = strdup(("SERVER_NAME=" + server.getIP()).c_str());  // From Server::_ip
    _env[i++] = strdup(("SERVER_PORT=" + server.getPort()).c_str());  // From Server::_port
...
void CGI::processRequestBody(const ClientRequest& request)
{
    if (_request_method == "GET") {
        _processed_body = "";  // No body for GET
...
void CGI::initPollFds()
{
    _poll_fds.clear();
    if (_in_pipe[1] != -1 && !_input_done) 
    {
        struct pollfd in_fd;
        in_fd.fd = _in_pipe[1];
        in_fd.events = POLLOUT;
        _poll_fds.push_back(in_fd);
    }
    if (_out_pipe[0] != -1 && !_output_done) 
    {
        struct pollfd out_fd;
        out_fd.fd = _out_pipe[0];
        out_fd.events = POLLIN;
        _poll_fds.push_back(out_fd);
    }
}

// Adjusted to use Server (instead of InfoServer)
...
void Server::handleCGIRequest(const ClientRequest& request, const std::string& uri, int fd, const Server& server) 
{
    try 
    {
        CGI* cgi = new CGI(request, "/tmp/uploads", server);  // Updated to use Server
        CGITracker tracker = {cgi, fd, cgi->getProcessedBody()};
        cgi_map[cgi->getInPipeWriteFd()] = tracker;
        cgi_map[cgi->getOutPipeReadFd()] = tracker;

        const std::vector<struct pollfd>& cgi_fds = cgi->getPollFds();
        poll_sets.insert(poll_sets.end(), cgi_fds.begin(), cgi_fds.end());
...
void Server::handleNonCGIRequest(const std::string& method, const std::string& uri, Server server, 
                                 const ClientRequest& request, const char* str, int size, int fd) 
{
    ...
    if (method == "GET")
        response = serverResponse.responseGetMethod(server, request);
    else if (method == "POST") 
        response = serverResponse.responsePostMethod(server, request, str, size);
    else if (method == "DELETE") 
        response = serverResponse.responseDeleteMethod(server, request);
    else 
    {
...
void Server::serverParsingAndResponse(const char *str, Server server, int fd, int size) 
{
    ...
    try 
    {
        ...
        if (uri.find(".py") != std::string::npos && (method == "GET" || method == "POST"))
            handleCGIRequest(request, uri, fd, server);
        else 
            handleNonCGIRequest(method, uri, server, request, str, size, fd);
    } 
...
void Server::initializeServerSockets(Server server, int* arraySockets) 
{
    // Assuming two ports for now—adjust if Server supports multiple ports differently
    arraySockets[0] = createServerSocket(server.getPort().c_str());
    if (arraySockets[0] < 0) {
        throw std::runtime_error("Failed to initialize server socket");
    }
    // For simplicity, using one port—extend if needed
    arraySockets[1] = -1;  // Placeholder; adjust if Server::_port becomes a vector
}
...
void Server::handleClientSocketEvent(int fd, Server server, std::vector<struct pollfd>::iterator& it) 
{
    ...
    if (bytesRecv == 0 || bytesRecv == -1) {
        ...
        serverParsingAndResponse(full_buffer.c_str(), server, fd, totBytes);
        ++it;
    } else {
        ++it;
    }
...
void Server::setupServerSockets(Server server, int* arraySockets)
{
    initializeServerSockets(server, arraySockets);
    for (int i = 0; i < 1; i++)
    {  // Single port for now—extend if needed
        if (arraySockets[i] != -1)
...
bool Server::runPollLoop(Server server, int* arraySockets)
{
    ...
    if (returnPoll == -1) {
        ...
        dispatchEvents(server, arraySockets, returnPoll);
        return true;
    }
}

void Server::dispatchEvents(Server server, int* arraySockets, int returnPoll)
{
    for (std::vector<struct pollfd>::iterator it = poll_sets.begin(); it != poll_sets.end();)
    {
        if (it->revents & (POLLIN | POLLOUT))
        {
            ...
            else if (cgi_map.find(it->fd) != cgi_map.end()) 
            {
                ...
                handleClientSocketEvent(it->fd, server, it);
...
//TODO: one socket fails all server fails
void Server::startServer(Server server) 
{
    int arraySockets[2] = {-1, -1};  // Adjust size if Server supports multiple ports
    try
    {
        setupServerSockets(server, arraySockets);
        while (runPollLoop(server, arraySockets)) {
